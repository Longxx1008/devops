...
function getMember(k,id,projectId ) {
    ...
    // console.log('memeber url---------- ',url);
    nodegrass.get(url, function (data, status, headers) {
        var dataTemp = JSON.parse(data);
        // console.log('-------memeberdata-----',dataTemp);
        ...
        pool.query(sqlSelectMember,[], function (err, results) {
            if (err) {
                ...
                if (results.length >0) {
                    ...
                    if (userRole != role || username != userName) {
                        var sqlUpdate = "update pass_develop_project_members set userRole='" + role + "',userName='"+username+"'  where id=" + data.id;
