...
router.route('/getDetailByNumber').get(function(req,res){
    ...
    requestDetailService.getServiceType(1, 100, conditionMap,async function (result){
        var list={};
        var j=1;   //list中第0个元素存长度，第1个元素开始存各个微服务的详细记录。
        for (let i=0;i<result.total;i++){
         let result1=  await requestDetailService.getDetailMsgByNumberPromise(result.rows[i].micro_service_id,conditionMap.serial_number);
         console.log(result1)
         if(list[j]=result1){
                console.log("!!!"+result1.rows[0].micro_service_id)
                var totalTime=0;
                var totalStatus="正常";
