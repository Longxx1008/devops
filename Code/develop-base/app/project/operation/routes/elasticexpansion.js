...
router.route("/microServices").get(function(req, res) {
   ...
   // console.log("get microServiceId = "+microServiceId);
    var conditionMap = {};
    if(!appId){
        appId =1;
    }
    ...
    elasticExpansionService.findList(conditionMap,function(strategiesResult){
        var microConditionMap2 = {};
        if(microServiceId){
            microConditionMap2.id = microServiceId;
        }
        elasticExpansionService.findMicroServiceList(microConditionMap2,function(microServiceResult){
            var newResult = {
