...
exports.pageList = function(page, size, conditionMap, cb) {
    var sql = " SELECT t.*,DATE_FORMAT(date_add(t.updateDate, interval 8 hour),'%Y-%m-%d %H:%i:%s') as imageTime from pass_develop_image_info t where 1=1";
    var conditions = [];
...
exports.add = function(data,data_map,cb) {
    var sql = "insert into pass_develop_image_info(imageResource,imageName,channels,pictureName,pictureType,picture,appPicture,simpleIntroduction,detailIntroduction,catagory) values(?,?,?,?,?,?,?,?,?,?)";
